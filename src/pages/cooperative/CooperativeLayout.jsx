...
export default function CooperativeLayout({ bookings = [], workers = [], setWorkers, setBookings, services = [], setServices }) {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col md:flex-row">
      ...
      <div className="md:hidden bg-slate-950 border-b border-slate-800 p-4 flex items-center justify-between sticky top-16 z-30">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-emerald-600 flex items-center justify-center text-white font-bold">
            <Building2 className="w-5 h-5" />
          </div>
          <div>
            <h2 className="font-extrabold text-sm text-white leading-tight">Cooperative Admin</h2>
            <p className="text-[10px] text-emerald-400 font-semibold">{activeCoop.name}</p>
          </div>
        </div>
      ...
      {/* SIDEBAR NAVIGATION */}
      <aside className={`
        fixed inset-y-0 left-0 z-40 w-64 bg-slate-950 border-r border-slate-800/80 flex flex-col justify-between transition-transform duration-300 ease-in-out md:static md:translate-x-0
        ...
        <div className="p-4 space-y-6">
          ...
          <div className="hidden md:flex items-center gap-3 px-2 pt-2">
            ...
            <div>
              <div className="flex items-center gap-1">
                <span className="font-extrabold text-lg tracking-tight text-white">Co-op Portal</span>
                <span className="bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-[9px] font-bold px-1.5 py-0.5 rounded">
                  ADMIN
                </span>
              </div>
          ...
          <div className="bg-slate-900/90 p-3 rounded-2xl border border-slate-800 space-y-1.5">
            ...
            <select
              ...
              className="w-full bg-slate-950 border border-slate-700 text-emerald-400 font-bold text-xs rounded-xl py-2 px-2.5 focus:outline-none focus:ring-2 focus:ring-emerald-500"
            >
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <Icon className={`w-4 h-4 ${active ? 'text-white' : item.highlight ? 'text-emerald-400' : item.aiBadge ? 'text-purple-400' : 'text-slate-400 group-hover:text-emerald-400'}`} />
                    <span>{item.name}</span>
                  </div>

                  {item.badge !== undefined && (
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${active ? 'bg-white/20 text-white' : 'bg-slate-800 text-slate-300'}`}>
                      {item.badge}
                    </span>
                  )}
                  {item.aiBadge && (
                    <span className="bg-purple-500/20 text-purple-300 border border-purple-500/30 px-1.5 py-0.5 rounded text-[9px] font-bold">
                      AI PRO
                    </span>
                  )}
        ...
        <div className="p-4 border-t border-slate-800/80 space-y-3">
          <div className="bg-slate-900 p-3 rounded-xl border border-slate-800 space-y-1">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold text-slate-300">Backend API</span>
              <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${isBackendLive ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-amber-500/20 text-amber-400 border border-amber-500/30'}`}>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-900">
        ...
        {/* TOP BAR */}
        <header className="bg-slate-950/60 border-b border-slate-800/80 px-6 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sticky top-0 z-20 backdrop-blur-md">
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-black text-white">{currentTabName}</h1>
              <span className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                <ShieldCheck className="w-3 h-3" /> Verified Co-op Admin
              </span>
            </div>
            <p className="text-xs text-slate-400 mt-0.5">
              Managing <strong className="text-slate-200">{activeCoop.name}</strong> • Zone: {activeCoop.zone}
            </p>
          </div>

          <div className="flex items-center gap-3">
            <div className="hidden lg:flex items-center gap-3 bg-slate-900 px-3 py-1.5 rounded-xl border border-slate-800 text-xs">
              <span className="text-slate-400">Reg No:</span>
              <span className="font-mono text-emerald-400 font-bold">{activeCoop.regNo}</span>
            </div>

            <div className="hidden lg:flex items-center gap-3 bg-slate-900 px-3 py-1.5 rounded-xl border border-slate-800 text-xs">
              <span className="text-slate-400">President:</span>
              <span className="text-white font-bold">{activeCoop.president}</span>
            </div>
